...
def main(args):
    ...
    ref_it = zip(*[itertools.chain(*fds) for fds in ref_fds])

    # group by length
    len2trans = defaultdict(list)
    len2refs = defaultdict(list)
    for x, y, ys in zip(src_it, trans_it, ref_it):
        length = len(x.split())
    ...
    n0 = sum(map(len, len2refs.values()))
    n1 = sum(map(len, len2trans.values()))
    if n0 != n1:
        raise RuntimeError("Lines mismatch.(n-ref={}, n-trans={})".format(n0, n1))

    maxlen = max(len2trans.keys())
    ...
    sorted_len2trans = [(length, len2trans[length]) for length in sorted(len2trans.keys())]
    sorted_len2refs = [(length, len2refs[length]) for length in sorted(len2trans.keys())]

    int2refs = OrderedDict()
    for lower, upper in zip(interval[:-1], interval[1:]):
        lines = []
        refs = []
        for (length, i_lines), (_, i_refs) in zip(sorted_len2trans, sorted_len2refs):
            if method == 'less':
                if length <= upper:
    ...
    for (lower, upper), lines in int2trans.items():
        n_diff = len(lines) - n
        n = len(lines)
