...
class optimizer:
    def __init__(self,inputs,outputs, cost,scopes, **option):
        ...
        if option["constraint"]:
            ...
            if method == "value":
                grads = clip_by_value(grads, value[0], value[1])
            if method == "norm":
                grads = clip_by_global_norm(grads, value)

        if option["nanguard"]:
            gnorm = global_norm(gradsref)
        ...
        if algorithm == "sgd":
            ...
            svar, pup = rmsprop_updates(params, vec, *rmsparam)
            ...
            svar, pup = rmsprop_momentum_updates(params, vec, *hparams)
            ...
            svar, pup = adadelta_updates(params, vec, *hparams)
        elif algorithm == "adam":
            alpha = theano.tensor.scalar()
            ...
            svar, pup = adam_updates(params, vec, *hparams)
        else:
